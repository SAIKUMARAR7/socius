...
def updateVote(votes, target, vote_type, question_or_answer):
    if question_or_answer == 'question':
        upvoted_targets = votes.values('upvoted_questions')
        downvoted_targets = votes.values('downvoted_questions')
    else:
        upvoted_targets =votes.values('upvoted_answers') 
        downvoted_targets =votes.values('downvoted_answers') 
...
def questionView(request, id):
    ...
    for answer in answers_serialized:
        ...
        if not current_user.is_authenticated:
            pass
        elif votes.objects.filter(id=answer['id'],user_id=current_user.id).values('upvoted_answers').count() > 0:
            answer['upvoted'] = True
        elif votes.objects.filter(id=answer['id'],user_id=current_user.id).values('downvoted_answers').count() > 0:
    ...
    if not current_user.is_authenticated:
        pass
    elif votes.objects.filter(id=question.id).values('upvoted_questions').count() > 0:
        upvoted = True
    elif votes.objects.filter(id=question.id).values('downvoted_questions').count() > 0:
        downvoted = True
...
def newView(request):
    ...
    if request.method != 'POST':
        render(request, 'main/new.html', {'current_user': current_user})
    
    form = QuestionForm(request.POST)
    if not form.is_valid() :
        return render(request, 'main/new.html', {'current_user': current_user})
    ...
    q.save()
    return HttpResponseRedirect('/question/my_questions')
...
def myAnswersView(request):
    ...
    return render(request, 'main/my_answers.html',
...
def myQuestionsView(request):
    ...
    return render(request, 'main/my_questions.html',
